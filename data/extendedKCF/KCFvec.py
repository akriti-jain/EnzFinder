...
class KCFvec(Compound):
                    ...
                    if self._check_in_ring(atom):
                        if Explicit == 3:
                            ...
                            for neighbor in atom.GetNeighbors() :
                                if (str(self.mol.GetBondBetweenAtoms(atom.GetIdx(),neighbor.GetIdx()).GetBondType())=='DOUBLE') and (not self._check_in_ring(neighbor)):
                                    kegg_atom = "C2z"
                                    break
                ...
                #         kegg_atom = "N0"
                    ...
                    for bond in atom.GetBonds():
                        if bond.GetBondType() == Chem.rdchem.BondType.TRIPLE:
                            ...
                            num_double_bond +=1
                    if num_triple_bond == 1 :
                        kegg_atom = "N3b"
                    elif num_double_bond == 2 :
                        kegg_atom = "N3c"
                    else :
                        kegg_atom = "N0"
    ...
    def _get_label_S(self, atom):
        num_neighbor_oxygen_atoms = 0
        num_neighbor_sulfur_atoms = 0
        num_neighbor_oxygen_bond_type = []
        neighbor_oxygen_explicit = []
        num_neighbor_oxygen_singleBond_explicitTwo = 0
        Explicit = atom.GetExplicitValence() - atom.GetNumExplicitHs()
        kegg_atom = ""
        for neighbor in atom.GetNeighbors():
            if neighbor.GetSymbol() == "O":
                num_neighbor_oxygen_atoms += 1
                neighbor_oxygen_explicit.append((neighbor.GetExplicitValence() - neighbor.GetNumExplicitHs()))
                num_neighbor_oxygen_bond_type.append(str(self.mol.GetBondBetweenAtoms(atom.GetIdx(),neighbor.GetIdx()).GetBondType()))
                if (str(self.mol.GetBondBetweenAtoms(atom.GetIdx(),neighbor.GetIdx()).GetBondType())=='SINGLE')and ((neighbor.GetExplicitValence() - neighbor.GetNumExplicitHs())==2) :
                    num_neighbor_oxygen_singleBond_explicitTwo +=1
            elif neighbor.GetSymbol() == "S":
                num_neighbor_sulfur_atoms += 1

        if self._check_in_ring(atom):
            if (num_neighbor_sulfur_atoms>=1) and (Explicit==2):
                kegg_atom = "S3x"
            elif (num_neighbor_sulfur_atoms==0) and (Explicit==2) and (len(atom.GetNeighbors())==2):
                kegg_atom = "S2x"
        ...
        if len(kegg_atom)==0:
            if (num_neighbor_sulfur_atoms>=1) and (Explicit==2) :
                ...
                if (Explicit==1) or ((atom.GetNumExplicitHs()==2) and (Explicit==0)) :
                    kegg_atom = "S1a"
                elif (Explicit==2) :
                    if (Explicit - len(atom.GetNeighbors())) == 1:
                        kegg_atom = "S5b"
                    elif (Explicit - len(atom.GetNeighbors())) == 0:
                        kegg_atom = "S2a"
                    else :
                        kegg_atom = "S0"
                elif (Explicit==3) :
            ...
            elif (num_neighbor_oxygen_atoms==1) :
                if Explicit == 2 :
                    if (neighbor_oxygen_explicit[0] == 1) :
                        kegg_atom = "S1b"
                    else :
                        kegg_atom = "S0"
                elif Explicit == 3 :
                    if (neighbor_oxygen_explicit[0] == 1)and (Explicit - len(atom.GetNeighbors()) == 1):
                        kegg_atom = "S1c"
                    elif (neighbor_oxygen_explicit[0] == 1)and (Explicit - len(atom.GetNeighbors()) == 0):
                        kegg_atom = "S1d"
                    else :
                        kegg_atom = "S0"
                elif (Explicit == 4) and (num_neighbor_oxygen_bond_type[0] == 'DOUBLE' ) :
                    kegg_atom = "S5a" 
                else :
                    kegg_atom = "S0"

            elif (num_neighbor_oxygen_atoms==2) :
                if (num_neighbor_oxygen_bond_type.count('DOUBLE') == 2) and (Explicit==6) :
                    kegg_atom = "S5c"
                elif (num_neighbor_oxygen_bond_type.count('DOUBLE') == 1) and (neighbor_oxygen_explicit.count(1) == 1) :
                    kegg_atom = "S6a" 
                elif (num_neighbor_oxygen_bond_type.count('DOUBLE') >=1) and (len(atom.GetNeighbors())==2):
                    kegg_atom = "S6a"
                else :
                    kegg_atom = "S0"

            elif (num_neighbor_oxygen_atoms==3) :
                if (num_neighbor_oxygen_bond_type.count('DOUBLE') == 2) and (num_neighbor_oxygen_singleBond_explicitTwo==1):
                    kegg_atom = "S7a"
                elif (num_neighbor_oxygen_bond_type.count('DOUBLE') == 1) and (num_neighbor_oxygen_singleBond_explicitTwo==2):
                    kegg_atom = "S7b"
                elif (len(atom.GetNeighbors()) in [3,4]) and (num_neighbor_oxygen_bond_type.count('DOUBLE') >= 1) :
                    kegg_atom = "S4a"
                else :
                    kegg_atom = "S0"

            elif (num_neighbor_oxygen_atoms==4) :
                kegg_atom = "S7c"
    ...
    def _get_label_else(self, atom):
        if atom.GetSymbol() == "F":
            kegg_atom_label = self._create_kegg_atom_label("F", atom)
        elif atom.GetSymbol() == "Cl":
            kegg_atom_label = self._create_kegg_atom_label("Cl", atom)
        elif atom.GetSymbol() == "Br":
            kegg_atom_label = self._create_kegg_atom_label("Br", atom)
        elif atom.GetSymbol() == "I":
            kegg_atom_label = self._create_kegg_atom_label("I", atom)
        elif atom.GetSymbol() == "*":
